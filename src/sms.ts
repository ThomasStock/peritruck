...
import { t } from "./i18n";
import { type Lang as DriverLang, t as driverT } from "./kiosk/i18n";
const esc = (s: string) =>
  s.replace(
    /[&<>"']/g,
    (c) =>
...
/** The message body, one entry per paragraph, in the language the driver chose
 * on the kiosk. The PIN sits alone on the middle line. */
export function smsLines(
  booking: string,
  pin: string,
  dock = 3,
  lang: DriverLang = "en",
  ...
  return [
    driverT(lang, "smsLine1", { booking }),
    driverT(lang, "smsPin", { pin }),
    driverT(lang, "smsLine3", { dock: dockLabel(dock) }),
  ];
}
...
export function smsBannerHtml(
  booking: string,
  pin: string,
  dock = 3,
  lang: DriverLang = "en",
): string {
  const body = smsLines(booking, pin, dock, lang).map(esc).join("\n");
  return `<button type="button" class="sms-banner__close" aria-label="${esc(t("sms.dismiss"))}">${closeIcon}</button><span class="sms-banner__icon">${messagesIcon}</span><span class="sms-banner__text" lang="${lang}"><span class="sms-banner__row"><b>${esc(SMS_SENDER)}</b><time>${esc(driverT(lang, "smsNow"))}</time></span><span class="sms-banner__body">${body}</span></span>`;
}
/** A handset opened on the Messages thread with the Peripass SMS. The phone is
 * the driver's, so its chrome speaks the driver's language too. Sizes inside
...
export function phoneHtml(
  ...
  const lines = smsLines(booking, pin, dock, lang);
  ...
  return `<div class="phone" role="img" aria-label="${esc(t("sms.phoneAria", { sender: SMS_SENDER, body: lines.join(" ") }))}"><div class="phone__body"><div class="phone__screen" lang="${lang}"><div class="phone__status"><span class="phone__clock">${esc(now)}</span><span class="phone__island"></span><span class="phone__signal">${signalIcon}${wifiIcon}${batteryIcon}</span></div><div class="phone__head"><span class="phone__back">${chevronLeft}</span><span class="phone__avatar">${esc(SMS_SENDER[0])}</span><span class="phone__contact">${esc(SMS_SENDER)}${chevronRight}</span></div><div class="phone__thread"><span class="phone__stamp"><b>${esc(driverT(lang, "smsToday"))}</b> ${esc(receivedAt)}</span><div class="phone__bubble"><p>${esc(intro)}</p><p class="phone__pin">${esc(pinLabel)} <b>${esc(pin)}</b></p><p>${esc(outro)}</p></div></div><div class="phone__compose"><span class="phone__plus">+</span><span class="phone__field">${esc(driverT(lang, "smsCompose"))}</span></div><span class="phone__homebar"></span></div></div></div>`;
}
